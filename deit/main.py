...
def main(args):
    ...
    if args.data_set == 'IMNET':
        # dataset_train, dataset_val = ImageNetData(
        #     args.data_path, 
        #     -1,
        #     return_dataset=True
        # )
        ...
        train_root = r'/images/PublicDatasets/imagenet/train/'
        dataset_train = torchvision.datasets.ImageFolder(train_root, transform=transform)
        val_root = r'/images/PublicDatasets/imagenet_shared/val/'
        dataset_val = torchvision.datasets.ImageFolder(val_root, transform=transform)
        args.nb_classes = 1000
    elif args.data_set == 'IMNETFIX':
        args.nb_classes = 1000
        # args.num_channel = 1
        # args.extensions = (".pt")
        ...
        # dataset_train, dataset_val = imagenetfix_data(args, defaultValues)
        # print(f'dataset size {len(dataset_train)}, {len(dataset_val)}')
        # # dataset_val = range(dataset_sizes['val'])
        # train_files = glob.glob(os.path.join(defaultValues['image_dir'], "train/*/*16x16.pt"))
        # val_files = glob.glob(os.path.join(defaultValues['image_dir'], "val/*/*16x16.pt"))
        # print(f'train & val patches size {len(train_files)}, {len(val_files)}')
    ...
    if args.data_set != 'COCO18TP':
        ...
        print(f'data_loader_train {len(data_loader_train)}')
    ...
    if (args.data_set == 'COCO18TP') or (args.data_set == 'IMNETFIX'):
        model = create_model(
            args.model,
            pretrained=False,
            num_classes=args.nb_classes,
    ...
    if args.finetune:
        ...
        extra_tokens = pos_embed_checkpoint[:, :num_extra_tokens]  # CLS token and distillation token
        # only the position tokens are interpolated
        pos_tokens = pos_embed_checkpoint[:, num_extra_tokens:]
        if (args.data_set != 'IMNETFIX') and (args.data_set != 'COCO18TP'):
            pos_tokens = pos_tokens.reshape(-1, orig_size, orig_size, embedding_size).permute(0, 3, 1, 2)
            pos_tokens = torch.nn.functional.interpolate(
                pos_tokens, size=(new_size, new_size), mode='bicubic', align_corners=False)
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
        else:
            # interpolate to new num_patches. Original image: 480x480 // 16x16 = 30x30
            pos_tokens = pos_tokens.permute(0, 2, 1)  # batch, num_patch=900, embedded dim
            pos_tokens = torch.nn.functional.interpolate(
                pos_tokens, size=num_patches, mode='linear', align_corners=False)
            pos_tokens = pos_tokens.permute(0, 2, 1)  # batch, embeded dim, new num_patch=10
        
        new_pos_embed = torch.cat((extra_tokens, pos_tokens), dim=1)
        checkpoint_model['pos_embed'] = new_pos_embed

        if args.no_pos_embed:
            checkpoint_model['pos_embed'].zero_
