...
class IdentityDataset(torch.utils.data.Dataset):
    def __init__(self, img_list, image_size):
    ...
    def __getitem__(self, idx):
        ...
        return self.img_transform(img), self.img_transform(img)
...
def evaluate(net, patch_size, n_patches, criterion, dataloader, device, amp):
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            image = pypatchify.patchify_to_batches(image, (1, patch_size, patch_size), batch_dim=0)
            mask_true = pypatchify.patchify_to_batches(mask_true, (1, patch_size, patch_size), batch_dim=0)
...
def train_model(
    ...
    train_set = IdentityDataset(train_files, image_size)
    val_set = IdentityDataset(val_files, image_size)
    ...
    for epoch in range(epoch_start, epochs + 1):
        ...
        with tqdm(total=n_train*n_patches, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                # save_image(targets, args.output_dir + 'sample_target.jpg')
                # print(f'images shape {images.shape}')
                images = pypatchify.patchify_to_batches(images, (1, patch_size, patch_size), batch_dim=0)
                # save_image(images[100], args.output_dir + 'sample_image_p100.jpg')
                # print(f'images shape {images.shape}')
                targets = pypatchify.patchify_to_batches(targets, (1, patch_size, patch_size), batch_dim=0)
                # print(f'targets shape {targets.shape}')
                # save_image(targets[100], args.output_dir + 'sample_target_p100.jpg')

                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    predictions = model(images)
                    print(f"predictions shape {predictions.shape}")
                    # save_image(predictions[100], args.output_dir + 'sample_predict_p100.jpg')
                    if model.n_classes > 1:
                        loss = criterion(predictions.squeeze(1), targets.float())
                        # loss += dice_loss(F.sigmoid(predictions.squeeze(1)), targets.float(), multiclass=False)
                    else:
                        loss = criterion(predictions, targets)
                        # loss += dice_loss(
                        #     F.softmax(predictions, dim=1).float(),
                        #     F.one_hot(targets, model.n_classes).permute(0, 3, 1, 2).float(),
                        #     multiclass=True
                        # )
                ...
                if (global_step > 5) and (global_step % (n_train // batch_size) == 0):
                    ...
                    #             'pred': wandb.Image(predictions.argmax(dim=1)[0].float().cpu()),
                    #         },
                    #         'step': global_step,
                    #         'epoch': epoch,
                    #         **histograms
...
def get_args():
    ...
    parser.add_argument('--output-dir', '-o', type=str, default='/work/scratch/tnguyen/unet/identity_encoder/0/', help='Dir path to save checkpoint')
    ...
    parser.add_argument("--encoder", default="unet2", type=str,
        help='Encoder type: "unet", "unet2", "linear". Default: "unet2"')
...
if __name__ == '__main__':
    ...
    model = UNet2(n_channels=1, n_classes=args.classes, bilinear=args.bilinear)
